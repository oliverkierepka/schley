...
$(document).bind("mobileinit", function() {    
    ...
    $.event.special.swipe.verticalDistanceThreshold = (screen.availHeight) / 3; // (default: 75) (pixels) – Swipe vertical displacement must be less than this.
...
$(document).on('submit', '#greetingsformular', function(event) {
   ...
   if($("#reciever_m").val() != "" && $("#sender_m").val() != "" && $("label[for='privacypolicy']").hasClass("ui-checkbox-on")) {
       ...
       $.ajax({
           ...
           if($("#reciever_m").val() == "") {return false;
                alert("Bitte geben Sie einen Empfänger für die Grußkarte an!");
               
           }else if($("#sender_m").val() == "") {return false;
                alert("Bitte geben Sie Ihre E-Mailadresse an!");
           }else if(!$("label[for='privacypolicy']").hasClass("ui-checkbox-on")) {return false;
           ...
           alert("Es ist ein Fehler aufgetreten! Wir bitten dies zu entschuldigen. Wenn der Fehler weiterhin auftritt dann schreiben Sie bitte ein E-Mail an app@schley.de. Danke für Ihre Mithilfe.");
                   ...
                   $.each(data.reminders, function(i,reminder){
                       ...
                       if(reminderOffsetDate.within(weekRange)) {
                          ...
                          reminderElementTabletContent =  '<div class="item" id="reminderTabletContent_'+reminder.rm_id+'"><div class="reminder_profile_detail"><div class="reminderprofile-pic-wrapper"><img height="100" class="reminderprofile-pic" src="'+rmImage+'" height="150" width="150" alt="'+reminder.rm_name+'"></div><div class="reminderprofile-text"><h1>'+reminder.rm_name+'</h1><p class="ui-li-desc">'+finalDate+'<br />'+reminderTypeText+'<br />' + reminderText + '</p><a href="" data-role="button" data-icon="delete" data-mini="true"  class="delete reminderDeleteButton" title="Diesen Eintrag löschen">Löschen</a></div><a href="#greetings" class="button greeting ui-link"><span data-icon="greeting" class="icon greeting"></span><span class="btn-text">Grusskarte versenden</span></a><div class="geschenke_tablet present'+reminder.rm_id+'"></div></div></div>';    
...
function calculateRoute() {
    if( $("input#from").val() != "") {
        ...
        if (start == '' || end == '') {
            ...
            alert("Bitte geben Sie Ihren Standort an");
            $("input#from").focus();
    }
}
               ...
               if(eventName22.val() != "" && eventDate22.val() != "" && eventtypeChecked22 == true) {
                   ...
                   $.ajax({
                          ...
                          //alert("success now hiding loading message");
                            if(data.status = "ok") {
                                $.mobile.hidePageLoadingMsg(); // This will show ajax spinner
                                loadReminders();
...
$( document ).on( "pageinit", "#reminder", function() {
    ...
    if(!calImportState) {
        window.setTimeout(function() {importCalendar()}, 4000);
    }
    ...
    function importCalendar() {
        ...
        if ( calendarConfirm ) {
            calImportSuccess = function(message) { 
                ...
                $.ajax({
                       ...
                       success: function(data){
                          $.mobile.hidePageLoadingMsg();
                           alert( "Kalendereinträge wurden erfolgreich importiert: " + data );
                       ...
                       error: function(xhr, textStatus, errorThrown) {
                           $.mobile.hidePageLoadingMsg(); // This will show ajax spinner
                           alert("Beim Kalenderimport sind Fehler aufgetreten! Bitte legen Sie Reminder manuell an");
                           window.localStorage.setItem("calImported", 1);
                           return false;
                       }
                });
            }
            calImportError = function(message) { 
                alert("Es konnten keine Events importiert werden. Bitte legen Sie Ihre Event-Reminder manuell an"); 
                window.localStorage.setItem("calImported", 1);
            }
...
function deleteReminder(rmid, listItem, transition) {
    ...
    $.ajax({
           ...
           error: function(data){
                //alert("Der Eintrag konnte nicht gelöscht werden. Bitte versuchen Sie es zu einem späteren Zeitpunkt erneut.");
           }
...
function coupon_success(data) {
        ...
        if(cp_valid) {
                ...
                if(signed == true) {
                    ...
                    aktuellerCouponContent = '<h2 class="centered" style="margin-top:20px">Ihr aktueller Coupon</h2><p class="centered">Gültig bis '+ cpEndDate +'</p><div class="owl-carousel" id="couponswiper"><div  class="item '+disableCoffee+'" id="coffeeCouponSlide"><img src="http://www.schley.de/coupons_app/'+ cp_coffeeimg +'" alt="" id="kaffeecoupon" /><a href="" data-role="button" data-corners="false" data-shadow="true" id="coffee_button" class="fakebutton couponbuttons" >Einlösen</a></div><div class="item '+disableShopping+'" id="shoppingCouponSlide"><img src="http://www.schley.de/coupons_app/'+ cp_shoppingimg +'" alt="" /><a href="" data-role="button" data-corners="false" data-shadow="true" id="shopping_button" class="fakebutton couponbuttons" >Einlösen</a></div></div>';
                     ...
                     $("#schleycoupon_btn .ui-btn-text").text("Coupon abholen");
                }            
...
function useCoupon(e) {
    var used = e.currentTarget.id;
    used = used.replace("_button", "");
    //console.log(used);
    var uid = window.localStorage.getItem("uid");
    var password = window.localStorage.getItem("password");
    ...
    $.ajax({
           ...
           cache: false,
           dataType: 'json',
           data: { uid: uid, pswd: password, used: used },
           beforeSend: function() {
    ...
    if(e) {
        ...
        if(callFrom == "click") {
            if(e.currentTarget.id == "coffee_button") {
                ...
                alert("Der Einkaufsgutschein wurde eingelöst! Vielen Dank!");
            }
        }
        if(coffeeused2 == 1) {
            $("#coffee_button .ui-btn-text").text("eingelöst");
            //$("#coffee_button").button("refresh");
            $("#coffeeCouponSlide").addClass("used");
        }
        if(shoppingused2 == 1) {
            $("#shopping_button .ui-btn-text").text("eingelöst");
            //$("#shopping_button").button("refresh");
...
$(document).on("pagebeforeshow", function() {
     connectionCheck();
     if( $.mobile.activePage.attr("id") != "loggedout" && $.mobile.activePage.attr("id") != "login" && $.mobile.activePage.attr("id") != "registrieren" && $.mobile.activePage.attr("id") != "calsuccess" && $.mobile.activePage.attr("id") != "calerror" && $.mobile.activePage.attr("id") != "registersuccess") {
         loggedInCheck();
     }
 });
...
function removePhoto() {
 ...
 //Greetings sending
        //alert( getUriVars("http://example.com?test=1&freak=2") );
        
        //sessionStorage.setItem("sm/rm/se")
        
        //if( sessionStorage.getItem("sm") == localstorage/sessionstorage.getItem("mail") )     ...(user allowed to send the ecard)
            //redirect to #greetings_send page ("ecard wird an sessionStorage.getItem("rm") gesendet")
                //call ajax send function (confirm.php) onpagebeforeshow (ajax/json/async) with sessionStorage.getItem("sm/rm/se") vars
                    //on success: redirect to greetings_success
                        //if status: ok
                            //redirect to greetings_success
                        //if status: error
                            //redirect to greetings_error
                        //if status: invalid
                            //redirect to greetings_bereitsgesendet
                    //remove sessionstorage sm, rm, se
